 ...
 *
 */
class ESHttpClient(endpoint: String, key: String, region: String, accessKeyId: String, service: String)
  ...
  private def awsPost(uriSuffix: String, data: Map[String, String]): Future[HttpResponse] = {
    ...
    val request = HttpRequest(method = HttpMethods.POST, uri = endpoint + uriSuffix, entity = entity)
    val authRequestFuture: Future[HttpRequest] = SignRequestForAWS.addAuthorizationHeader(request, key, region, accessKeyId, service)
    authRequestFuture.flatMap{req =>
      Source.single(req).via(outgoingConn).runWith(Sink.head)
    }
  }
}
