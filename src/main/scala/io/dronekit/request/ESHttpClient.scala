...
class ESHttpClient(endpoint: String)(implicit system: ActorSystem, materializer: ActorMaterializer) {
  ...
  def indexDocument(index: String, docType: String, docId: Option[String] = None, data: Map[String, Any]): Future[HttpResponse] = {
    val documentKey = if (docId.isDefined) s"/${docId.get}" else ""
    val uri = s"/$index/$docType$documentKey"
    post(uri, data)
  }

  private def post(uri: String, data: Map[String, Any]): Future[HttpResponse] = {
